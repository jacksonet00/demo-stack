...
class Register(graphene.Mutation):
  ...
  def mutate(root, info, input):
    output = AuthOutput()
    errors = []
    user = db_session.query(UserModel).filter(UserModel.username == input.username).first()
    if user:
      errors.append(InputError(field='username', message='already taken'))
    if len(input.username) < 3:
      errors.append(InputError(field='username', message='must be at least 3 characters'))
    if len(input.password) < 3:
      errors.append(InputError(field='password', message='must be at least 3 characters'))
    output.errors = errors
    if not errors:
      hashed_password = pbkdf2_sha256.hash(input.password)
      new_user = UserModel(username=input.username, password=hashed_password)
...
class Login(graphene.Mutation):
    ...
    def mutate(root, info, input) :
        ...
        user = db_session.query(UserModel).filter(UserModel.username == input.username).first()
        if not user:
          errors.append(InputError(field='username', message='does not exist'))
